...
1. 애플리케이션 서버의 상세 정보 페이지로 이동합니다.
2. "세션 관리" 링크를 클릭합니다.
3. "분산 환경 설정"에서 세션 서버를 지정합니다.
4. 세션 복제 모드를 선택합니다 (예: 클라이언트-서버 모드).
...
LibriX는 클라이언트-서버 모드의 세션 복제를 지원합니다:

**클라이언트 역할 (애플리케이션 서버)**
- 사용자 요청을 처리하고 세션 데이터를 생성합니다.
- 세션 데이터를 세션 서버(서버 역할)에 복제합니다.
- 로컬 캐시에 세션 데이터의 복사본을 유지하여 성능을 최적화합니다.

**서버 역할 (세션 서버)**
- 애플리케이션 서버(클라이언트 역할)로부터 세션 데이터를 수신하여 저장합니다.
- 세션 데이터의 백업 복사본을 유지합니다.
- 애플리케이션 서버 장애 시 세션 데이터를 제공합니다.

**장점:**
- 역할을 명확하게 구분하여 관리가 용이합니다.
- 각 애플리케이션 서버의 메모리 사용량을 줄입니다.
- 성능이 향상됩니다.
- 백업 서버를 재시작해도 애플리케이션 서버에 영향을 주지 않습니다.
- 하드웨어 사양이 서버마다 다른 경우에 적합합니다.

**단점:**
- 애플리케이션을 실행하는 서버 외에 추가 서버를 구성하고 유지 관리해야 합니다.
- 단일 장애점을 방지하기 위해 여러 세션 서버를 구성해야 합니다.
...
- 세션 서버 간에 세션 데이터를 균등하게 분산시킵니다.
- 애플리케이션 서버가 세션 서버를 선택할 때 부하 분산 알고리즘을 사용합니다.
