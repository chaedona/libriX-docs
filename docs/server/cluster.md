...
- 타임아웃: 지정된 시간 내에 종료되지 않으면 강제 종료할 수 있습니다.
- 순차 중지: 클러스터 멤버를 역순으로 중지하여 안전한 종료를 보장합니다.
...
이 옵션을 선택하면 새로운 애플리케이션 서버를 생성하면서 동시에 클러스터에 추가할 수 있습니다. (현재 화면에는 표시되지 않음)
...
**참고**: 멤버를 제거해도 서버 자체는 삭제되지 않으며, 독립적인 서버로 계속 존재합니다.
...
1. 애플리케이션 서버 생성 시 클러스터 옵션 선택
2. 또는 기존 서버의 구성에서 클러스터 지정
3. LibriX가 자동으로 `clusterMember-1.0` 기능 추가
4. 서버 재시작 후 클러스터에 조인
...
1. 서버를 중지합니다.
2. 서버 구성에서 클러스터 연결을 해제합니다.
3. LibriX가 자동으로 `clusterMember-1.0` 구성 제거
4. 서버를 재시작합니다 (독립 서버로).
...
- 잘못된 인증 정보
- 네트워크 방화벽 차단
- `clusterMember-1.0` 기능 미활성화

**해결방법**
- Collective Controller 가용성 확인
- server.xml의 `<collective>` 구성 확인
- 방화벽에서 필요한 포트 개방 (일반적으로 9443)
- `clusterMember-1.0` 기능이 활성화되어 있는지 확인
