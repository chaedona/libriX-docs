# JDBC 제공자 관리
JDBC 제공자(JDBC Provider)는 애플리케이션이 데이터베이스에 연결하기 위해 필요한 JDBC 드라이버와 구성 정보를 관리하는 LibriX의 핵심 리소스입니다. JDBC 드라이버 클래스 및 라이브러리 관리, 데이터베이스 벤더별 드라이버 구성, 데이터소스의 기반 제공 방법을 설명합니다.

## 목차

- [개요](#개요)
- [JDBC 제공자 메인 화면](#jdbc-제공자-메인-화면)
- [JDBC 제공자 생성](#jdbc-제공자-생성)
- [JDBC 제공자 상세 화면](#jdbc-제공자-상세-화면)
- [JDBC 제공자 삭제](#jdbc-제공자-삭제)
- [Liberty server.xml 구성](#liberty-serverxml-구성)
- [데이터베이스별 JDBC 드라이버 설정 가이드](#데이터베이스별-jdbc-드라이버-설정-가이드)
- [문제 해결](#문제-해결)
- [모범 사례](#모범-사례)
