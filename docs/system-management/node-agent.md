# 노드 에이전트 (Node Agent)
노드 에이전트는 각 노드에서 실행되는 관리 에이전트로, 배치 관리자와 노드 간의 통신을 중계하고 구성 동기화를 수행합니다. 노드 에이전트의 역할, 구성, 시작/중지 방법 및 문제 해결 방법을 설명합니다.

## 목차

- [개요](#개요)
- [노드 에이전트 화면](#노드-에이전트-화면)
- [Node Agent의 역할](#node-agent의-역할)
- [Node Agent 구성](#node-agent-구성)
- [Node Agent 시작 및 중지](#node-agent-시작-및-중지)
- [Node Agent 문제 해결](#node-agent-문제-해결)
