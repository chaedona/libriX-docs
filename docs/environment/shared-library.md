# 공유 라이브러리 관리
분석 완료했습니다! 다음 내용을 파일의 "# 공유 라이브러리 관리" 바로 아래에 삽입하세요:
markdown# 공유 라이브러리 관리

공유 라이브러리(Shared Library)는 여러 애플리케이션이 공통으로 사용하는 JAR 파일, 클래스, 리소스를 관리하는 메커니즘입니다. 공통 라이브러리 중앙 관리, 애플리케이션 간 코드 재사용, 라이브러리 버전 통합 관리 방법을 설명합니다.

## 목차

- [개요](#개요)
- [공유 라이브러리 메인 화면](#공유-라이브러리-메인-화면)
- [공유 라이브러리 개념](#공유-라이브러리-개념)
- [공유 라이브러리 사용 사례](#공유-라이브러리-사용-사례)
- [공유 라이브러리 생성](#공유-라이브러리-생성)
- [변경사항 저장 및 검토](#변경사항-저장-및-검토)
- [공유 라이브러리 수정 및 삭제](#공유-라이브러리-수정-및-삭제)
