...
- [Liberty server.env 구성](#liberty-server.env-구성)
- [문제 해결](#문제-해결)
- [모범 사례](#모범-사례)
...
## Liberty 변수 사용 (server.env)
HTTP_PORT=9080
HTTPS_PORT=9443
DB_HOST=wmddeem01
DB_PORT=3306
DB_NAME=myapp
APP_INSTALL_HOME=/was/wlp/apps
...
변수가 생성되면 Liberty의 server.env 파일이 자동으로 업데이트됩니다.
...
**서버 범위:**
```
파일: ${wlp.user.dir}/servers/MyTestServer/server.env
위치: <server> 요소 내부
```
...
범위: 호스트=localhost.localdomain
이름: DB_JDBC_URL
값: jdbc:mysql://127.0.0.01:3306/db_name
```
...
값: db_user
범위: 호스트=localhost.localdomain, 서버=MyTestServer
파일: /home/userid/libriX_home/wlp/usr/servers/MyTestServer/server.env
...
```bash
# 변수 존재 확인
grep "name=\"DB_HOST\"" ${server.config.dir}/server.env
