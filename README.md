...
│   └── aix73_apache_install_guide.md   # AIX 7.3 Apache HTTP Server 설치 가이드
├── server/                         # 서버 관리 문서
...
<details>
<summary><b>📋 설치 가이드 문서 보기</b></summary>
...
#### 0.2 [LibriX 설치 가이드](https://github.com/chaedona/libriX-docs/blob/main/docs/installation/libriX-install.md)

LibriX V2.0을 Open Liberty 또는 WebSphere Liberty 환경에 설치하는 방법을 설명합니다.

* 설치 미디어 구성 (Open Liberty / WebSphere Liberty)
* init.sh를 이용한 초기 설치
* Deployment Manager 및 Node Agent 생성
* 노드 연합 및 Admin UI 접속

#### 0.3 [Open Liberty Airgap(폐쇄망) Feature 설치 가이드](https://github.com/chaedona/libriX-docs/blob/main/docs/installation/open-liberty-airgap-install.md)

인터넷이 차단된 폐쇄망 환경에서 Open Liberty Feature를 설치하는 방법을 설명합니다.

* 인터넷 연결 서버에서 전체 Feature 자동 미러링 (download-liberty-features.sh)
* liberty-mirror 패키지 구성 및 폐쇄망 전송
* Python HTTP 서버 기반 오프라인 Feature 설치
* 버전 업그레이드 및 운영 중 Feature 추가

#### 0.4 [AIX 7.3 Apache HTTP Server 설치 가이드](https://github.com/chaedona/libriX-docs/blob/main/docs/installation/aix73_apache_install_guide.md)

인터넷이 차단된 에어갭(Air-Gap) 환경의 AIX 7.3 서버에 Apache HTTP Server를 설치하는 방법을 설명합니다.

* AIX Toolbox 오프라인 미러링 패키지(tar.gz) 준비 방법
* USB를 이용한 파일 전송 및 압축 해제
* rpm.rte 업데이트 및 DNF 설치
* 로컬 DNF 리포지토리 구성 및 httpd 설치
* 기동 확인 및 트러블슈팅 (6가지 오류 유형)
...
* ✅ **AIX 7.3 Apache HTTP Server 설치 가이드** 추가
* ✅ 에어갭(Air-Gap) 환경 기준 설치 절차 (8단계)
* ✅ AIX Toolbox 오프라인 미러링 및 로컬 DNF 리포지토리 구성
* ✅ 트러블슈팅 6가지 오류 유형 및 해결 방법
...
- 
### 2026-02-09 ⭐ 추가
- ✅ 트랜잭션 페일오버 섹션 추가 (Open Liberty 문서 참조)
- ✅ 1개 스크린샷 추가
